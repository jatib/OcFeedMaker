...
**/
	...
	function feedConstructor($start,$end,$key,$feedHead,$filename="merchant",$prefix=DB_PREFIX){
		
		$myfile = fopen($filename.".txt", "w")or die("Unable to open file!");
		$imgURL = "http://www.smartify.mx/image/cache/data/";
		...
		for ($i = $start; $i < ($end + $start ); $i++){
			...
			/*CATEGORY*/
			$category = "SELECT * FROM ".$prefix."oc_product_to_category WHERE product_id=\"$i\"";
			$category = mysqli_query($key,$category);
			$category = mysqli_fetch_row($category);
			$categoryId = "SELECT * FROM ".$prefix."oc_category_description WHERE category_id=\"category[1]$\"";
